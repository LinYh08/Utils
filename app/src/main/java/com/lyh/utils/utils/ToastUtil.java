...
public class ToastUtil {
     ...
     * @param msg
     */
    public static void showMessage(final int msg) {  //建议去掉这个方法,有可能传过来的不是资源ID,而是将Int等整型变量作为了参数传过来了,就会抛出Resources ClientRequestException
        showMessage(msg, Toast.LENGTH_SHORT);
    }
            ...
            public void run() {
                synchronized (synObj) { //加上同步是为了每个toast只要有机会显示出来
                    if (toast != null) {
                        ...
                        toast = Toast.makeText(BaseApp.getAppContext()
